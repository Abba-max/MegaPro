...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        saved = await _save_message(self.conversation, self.user, text)

        # Broadcast to all participants (including sender so they get the real id)
        ...
        # Also push a notification to the OTHER participant's notification channel
        other_id = (
            self.conversation.owner_id
            if self.user.id == self.conversation.client_id
            else self.conversation.client_id
        )
        await self.channel_layer.group_send(
            f"user_{other_id}",
            {
