...
def stats_view(request):
    ...
    estates_count  = Estate.objects.filter(status='published').count()
    users_count    = DjangoUser.objects.filter(is_active=True).count()
    reviews_count  = Review.objects.count()
    campuses_count = Estate.objects.filter(status='published').values('location').distinct().count()
    return Response({
